...
<% 
	...
	while(rs.next()) {
		String item_n = rs.getString(3);
		String id = rs.getString(2);
		String anotherstring = "select restaurant_name from restaurant where restaurant_id = '"+id+"'";
		PreparedStatement stat = conn.prepareStatement(anotherstring);
		ResultSet ns = statement.executeQuery();
		while(ns.next()) {
			String restaurant_n = ns.getString(2);
		%>
		<%= i+")" %>
		<a href = "Items.jsp?param=restaurant_n" ><%= restaurant_n%></a>
		<%= "," + item_n %>
		<br/>
		<hr>
		<%i++; %>
